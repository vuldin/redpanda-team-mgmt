...
This guide focuses on the role of a support team, and shows how to:

1. Configure Redpanda for TLS/SASL, and configure Redpanda Console for TLS/SASL/SSO
1. Gather details from teams within the broader organization
1. Modify SASL and role binding config based on gathered details
1. Provide info to the team for getting them up and running with Redpanda

## Install prerequisite tools

While this guide can be used for any deployment, also included are docker compose files which allow testing out these steps before applying other clusters. You will need [Docker](https://docs.docker.com/engine/install/) installed if you plan to use the compose files (otherwise skip those sections and focus instead on the Redpanda config and associated `rpk` commands).

Run the following commands to install dependencies. The final script `delete-data.sh` will set ownership to the redpanda (UID 101) user and also clear the Redpanda data directory (to reset between runs):
...
Fix ownership on Redpanda data directory so that it is owned by the `redpanda` user:

```
./fix-ownership.sh
```

Create an rpk admin profile:

```
rpk profile create team-mgmt-admin -s brokers=localhost:9092 -s tls.ca="$(realpath ./certs/ca.crt)" -s admin.hosts=localhost -s admin.tls.ca="$(realpath ./certs/ca.crt)" -s kafka_api.sasl.user=admin -s kafka_api.sasl.password=adminpassword -s kafka_api.sasl.mechanism=SCRAM-SHA-512
```
...
login:
  ...
  keycloak:
    ...
    tls:
      caFilepath: optional // If not set uses system cert pool
      certFilepath: optional
...
Start a 3-broker Redpanda cluster along with Redpanda Console:
